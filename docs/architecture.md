...
The experimental `/lab/spatial` route is deliberately outside this composition. `SpatialLab` owns native comparison controls and dynamically loads its direct Three.js component only when selected. CSS/GSAP and SVG prototypes remain scoped to lab components; the Three.js runtime owns its renderer lifecycle and fallback inside that boundary. No lab component is imported by `/`, no lab route is linked from production navigation, and the route is `noindex`, omitted from the sitemap, and covered by the `/lab/` robots boundary. See the [spatial design exploration](spatial-design-exploration.md) for the comparison and recommendation.
