...
The isolated spatial comparison is now complete. Its documented recommendation is a small CSS-perspective laptop threshold combined with layered SVG path depth; Three.js remains lab-only and is not approved for the production route. A future implementation issue should validate that hybrid against the existing journey timing before any broader spatial redesign.
